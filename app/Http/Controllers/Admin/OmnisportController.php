...
class OmnisportController extends Controller
{
    //
    public function omnisports()
    {
        $articles = Omnisport::latest()->get();
        return view('Admin.Pages.Omnisport.omnisport', compact('articles'));
    }

    //Afficher les omnisports sur l'acceuil
    public function showOmnisport()
    {
        $articles = Omnisport::with('author')
        ...
        $recentPosts = Omnisport::with('author')
            ->where(function ($query) {
                $query->where('publish_at', '<=', now())
        ...
        return view('Users.Pages.Omnisport.omnisport', compact('articles', 'recentPosts'));
    }


    public function showdetail($slug)
    {
        $article = Omnisport::where('slug', $slug)->firstOrFail();
        $article->increment('views');
        $recentPosts = Omnisport::latest()->take(8)->get();
        return view('Users.Pages.Omnisport.detail', compact('article', 'recentPosts'));
    }
    public function createOmnisport()
    {
        return view('Admin.Pages.Omnisport.addOmnisport');
    }

    public function storeOmnisport(Request $request)
    {
        $request->validate([
            ...
            'image.max'      => 'La taille maximale de l\'image est 9Mo.',
        ...
        //  Création de l’article Omnisport avec auteur
        $omnisport = Omnisport::create([
            'title'       => $request->title,
            'image'       => $path,
        ...
        //     Mail::to($subscriber->email)->send(new NewArticleMail($omnisport, 'omnisport'));
        // }

        return redirect()->route('omnisports')->with('success', 'Article ajouté avec succès !');
    }
    ...
    {
        $article = Omnisport::findOrFail($id);
