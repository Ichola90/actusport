...
@section('title', $articlesafrique->title . ' – InfoflashSports')
@section('description', Str::limit(strip_tags($articlesafrique->content), 160))
@section('keywords', 'sport, ' . $articlesafrique->categorie . ', InfoflashSports')

@section('og_title', $articlesafrique->title . ' – InfoflashSports')
@section('og_description', Str::limit(strip_tags($articlesafrique->content), 160))
@section('og_image', $articlesafrique->image ? asset($articlesafrique->image) : asset('assets/img/logo/logoif.png'))
@section('og_type', 'article')

@section('twitter_title', $articlesafrique->title . ' – InfoflashSports')
@section('twitter_description', Str::limit(strip_tags($articlesafrique->content), 160))
@section('twitter_image', $articlesafrique->image ? asset($articlesafrique->image) : asset('assets/img/logo/logoif.png'))

@section('content')
<style>
    .hero-img-wrapper {
        position: relative !important;
        /* pour que la catégorie soit sur l’image */
        width: 100% !important;
    ...
    .post-category {
        ...
        left: 10px !important;
        background-color: rgba(126, 217, 87, 0.8) !important;
        color: #fff !important;
...
// Auteur principal
$author = $articlesafrique->author ?? null;
...
<div class="page-title">
    <div class="breadcrumbs">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}"><i class="bi bi-house"></i> Accueil / Actusport</a></li>
                <li class="breadcrumb-item active current">{{ $articlesafrique->category ?? 'Article' }} - détail</li>
            </ol>
        </nav>
    </div>

    <div class="title-wrapper">
        <h1>{{ $articlesafrique->title }}</h1>
        <p>{!! Str::limit(strip_tags($articlesafrique->content), 100) !!}</p>
    </div>
</div>
<!-- Page Content -->
<div class="container">
    <div class="row">
        ...
        <div class="col-lg-8">
            <section id="blog-details" class="blog-details section">
                ...
                <article class="article " style="margin-bottom: 100px !important;">
                    <!-- Hero Image -->
                    <div class="hero-img-wrapper" data-aos="zoom-in">
                        @if($articlesafrique->image)
                        <img src="{{ asset($articlesafrique->image) }}"
                            alt="{{ $articlesafrique->title }}"
                            class="hero-article-img">
                        @else
                        <img src="https://via.placeholder.com/900x600?text=Article"
                            alt="No Image"
                            class="hero-article-img">
                        @endif

                        @if($articlesafrique->category)
                        <h6 class="post-category">
                            ActuSport / {{ $articlesafrique->category }}
                        </h6>
                        @endif
                    </div>
                    ...
                    <div class="article-content mt-2" data-aos="fade-up" data-aos-delay="100">
                        <div class="author-info d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="post-meta" style="display: flex; align-items: center; gap: 15px; font-size: 0.9rem; color: #555;">
                                <span class="date" style="display: flex; align-items: center; gap: 4px;">
                                    <i class="bi bi-calendar3"></i>
                                    {{ Carbon::parse($articlesafrique->created_at)->format('d/m/Y') }}
                                </span>
                            </div>
                        </div>

                        <div class="content">
                            {!! embedSocial($articlesafrique->content) !!}
                      @if($articlesafrique->tags)
                    <div class="article-tags mt-4 d-flex align-items-center gap-2">
                        <h6 class="fw-bold me-2">Mots-clés :</h6>
                        @foreach(explode(',', $articlesafrique->tags) as $tag)
                    ...
                    <div id="commentForm" class="mt-4 p-4 rounded shadow-sm bg-white border border-light" style="display: none;">
                        ...
                        <form action="{{ route('comments.store') }}" method="POST">
                            ...
                            <input type="hidden" name="commentable_id" value="{{ $articlesafrique->id }}">
                            <input type="hidden" name="commentable_type" value="{{ get_class($articlesafrique) }}">
            ...
            <div class="widgets-container">
                ...
                <div class="recent-posts-widget widget-item mt-4">
                    ...
                    <div class="post-item d-flex mb-3">
                        ...
                        <div>
                            <h4><a href="{{ route('actuafrique.detail', $post->slug) }}">{{ Str::limit($post->title, 30) }}</a></h4>
